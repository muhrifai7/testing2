...
export const destroy = async(req : Request,res:Response|any,next:NextFunction)=> {
    ...
    try {
        ...
        if(!department){
            const customError = new CustomError(404, 'General', 'Not Found', [`department with id:${id} doesn't exists.`]);
            return next(customError);
        }
        departmentRepository.delete(id);
        // res.customSuccess(200, 'User successfully deleted.', { id: user.id, name: user.name, email: user.email });
        return res.status(200).json({status: 200,message :"Department successfully deleted.",responseData : { id: department.id, name: department.name} });
    } catch (error) {
